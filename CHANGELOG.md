...
### Changed
- **PR-gated task completion with per-task releases** — `task-complete` for a standalone or parent task no longer merges directly into `main`. It now runs in two phases: Phase 1 commits the implementation, computes the task's own release version (fetched fresh against `origin/main` and aware of any other concurrently pending task's already-claimed version), pushes a `(pending vX.Y.Z · PR #NNN)`-annotated `CHANGELOG.md` entry directly to `main`, then pushes the branch and opens a code-only PR — the PR is also the task's release. Assisted mode stops there; Autonomous mode immediately self-merges (`gh pr merge --merge`) and falls through to Phase 2 in the same invocation. Phase 2, re-entrant, verifies the merge via `gh pr view` and cleans up (worktree removal, local-only branch force-delete via `-D`, since GitHub's own merge state — not git's local ancestry check — is now authoritative and squash merges never satisfy `-d`). The remote branch is never deleted, preserved as an audit trail. `task-start` and `task-complete`'s metadata pushes (task status, `PLANNING.md`, the pending changelog entry) go directly to `origin/main` immediately, with a bounded fetch-rebase-retry loop on collision, instead of waiting for `session-finish`. A new `PR Open` task status records the in-flight PR number. Child task completion is completely unaffected — no PR, no release, bookkeeping only, exactly as before. `post-merge-release.yml` (this repo's copy and the consumer template) gained a `concurrency` group so back-to-back per-task releases queue instead of racing.
...
### Removed
- **Committed dogfooding mirrors** — `.github/skills/`, `.github/agents/`, `.codex/agents/`, `.agents/skills/`, and `.claude/skills/` are no longer tracked in the repository. Following the v1.14.0 global-install shift, these were always regenerated by `make sync` from canonical `skills/`/`agents/` sources; they served only as stale duplicates. The build pipeline (`make sync`, `make test`, `make smoke-test`) continues to produce and verify them.

### Fixed
- **Worktree script repo-root resolution for global install** — `smaqit.utils.worktree` scripts now correctly locate the canonical `skills/` and `agents/` directories after the v1.14.0 global-install migration removed project-level copies. Stale project-level path references are repaired.
...
[1.14.1]: https://github.com/ruifrvaz/smaqit-extensions/compare/v1.14.0...v1.14.1
[1.14.0]: https://github.com/ruifrvaz/smaqit-extensions/compare/v1.13.0...v1.14.0
