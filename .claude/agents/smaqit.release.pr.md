...
- Claude Code operating on a pull-request branch
- Runs in GitHub Actions with limited credentials
- Creates pull request (cannot commit to `main` directly)
- Uses direct `git push` using the current environment's git/gh credentials for commits
...
- Pushes via direct `git push` using the current environment's git/gh credentials
...
- [ ] PR created/updated with changes via direct `git push` using the current environment's git/gh credentials
...
- Push authentication uses direct `git push` using the current environment's git/gh credentials
