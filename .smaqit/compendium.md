...
The design eliminates merge conflicts on `PLANNING.md`: when `task-start` and `task-complete` update task status, they write to main's copy directly — rather than to the worktree's copy or deferring to `session-finish`. The worktree is purely for source code changes.

Since v2.0.4 (task 036), these bookkeeping commits land on **local `main` only and are never pushed** — every worktree of a repository shares the same `.git` object database and ref namespace as the primary checkout, so a local commit is immediately visible everywhere with no network operation. This replaced an earlier design that pushed each bookkeeping commit to `origin/main` immediately with a bounded fetch-rebase-retry loop on collision; that direct push broke outright on any repo with required-PR-review branch protection on `main`, forcing several bookkeeping-only PRs per task. The re-read-before-write policy that replaced the retry loop still applies the same "never auto-resolve a genuine conflict, abort and report" rule, just against local refs instead of the network. See `smaqit.task-start`/`smaqit.task-complete`'s own "Local-Only Sessions" notes for the current mechanics.
...
**Phase 1** (Status `In Progress`): commits the implementation, computes the task's own release version via `release-analysis`'s Task mode (fetches `origin/main` fresh for the tag boundary and treats every version named by an open `Prepare release vX.Y.Z` PR title as already claimed), pushes the branch, opens a PR titled `Prepare release vX.Y.Z`, then writes the finished `## [X.Y.Z] - date` section directly on the task branch — folding in any bullets the implementer left under `[Unreleased]` — and commits and plain-pushes it. Nothing is written to `main`, the branch is never rebased, and the push is never forced. The task's status becomes `PR Open`, recording the PR number in a `pr:` frontmatter key. Assisted mode stops here; Autonomous mode immediately self-merges (`gh pr merge --merge`) and falls straight into Phase 2.

**Phase 2** (Status `PR Open`, re-entrant): confirms the PR actually merged via `gh pr view --json state,mergedAt` — never inferred any other way — merges `origin/main` into local `main`, flips status to `Completed`, removes the worktree, and force-deletes (`-D`, not `-d`) the **local** branch only. `-D` is required because GitHub's own merge confirmation is authoritative regardless of merge strategy, and a squash merge leaves the local branch tip unable to satisfy `-d`'s ancestry check. The remote branch is never deleted — it's kept indefinitely as an audit trail of every merged/released task.

Each PR is also that task's release: merging it is what triggers `post-merge-release.yml`'s existing tag + GitHub Release automation, just now firing per-task instead of per manually-triggered batch. Several tasks can be in flight at once, each claiming its version by its own PR title — so tags can land out of numeric order, and this is expected. Since v2.0.2, `release-analysis` accounts for that ordering explicitly (see "How does `release-analysis` locate the boundary for the current release?"). Two open PRs can still collide on `CHANGELOG.md` when both insert their `## [X.Y.Z]` section directly below `[Unreleased]` — the second to merge needs a manual conflict resolution on GitHub — and that is now the only changelog conflict the lifecycle can produce. See also: "How does a project get post-merge release automation (tag + GitHub Release) after installing smaqit-extensions?" and "Where is a task's changelog entry written, and how are concurrent versions kept distinct?"

A task's own file (its Description, Implementation Steps, Files to Create/Modify) is a plan written at task-creation time — it is not itself the completion mechanism and can go stale if it predates a later change to that mechanism. In particular, a task file should never instruct a manually-authored `CHANGELOG.md` release section: `task-complete` Phase 1 writes the `## [X.Y.Z]` section itself, and anything an implementer leaves under `[Unreleased]` is folded into it automatically. When a task file's own steps conflict with the currently-installed `task-complete`/`task-start` skill behavior, the installed skill is authoritative — verify by reading it directly rather than assuming the task file is current.

---

**Where is a task's changelog entry written, and how are concurrent versions kept distinct?**

On the task branch, by `task-complete` Phase 1, after the PR exists — never on `main`. The open PR's title (`Prepare release vX.Y.Z`) is the version claim: `release-analysis` Step 1e and `release-approval` Step 4b read `gh pr list --state open --limit 100 --json number,title` and treat every version named by a matching title as taken. Titles are filtered client-side, never with `--search`, because GitHub's search index is eventually consistent and can miss a PR opened seconds earlier. A closed-unmerged PR releases its version back to the pool; a never-tagged version is safe to reuse. `task-start` creates the owner branch from freshly fetched `origin/main`, so local `main`'s unpushed bookkeeping commits never enter a PR.

This replaced (task 039) the original task 027 design, where the entry was born on `main` as a `(pending vX.Y.Z · PR #NNN)`-annotated bullet under `[Unreleased]` and then transplanted into the branch by rebasing the branch onto `main` and promoting the bullet — a promotion step task 027's own Findings record as bolted on during self-review. Three structural problems retired with it. `[Unreleased]` had two owners on one line range — the branch, where implementers naturally write bullets, and `main`, where Step 12 wrote its own — so the rebase conflicted whenever both had touched it and hit the "never auto-resolve" hard stop. The claim registry had been dead since task 036 moved Step 12's commit to local `main` while Step 1e kept reading `origin/main:CHANGELOG.md`, so a concurrent task never saw another's claim. And the rebase onto local `main` dragged every unpushed bookkeeping commit into the release PR (verified on `origin/main`: `chore: start task 038`, `chore: create task 037`, and others arrived through PR branches), contradicting 036's own contract. A first attempt at a fix — an instruction telling implementers not to touch `[Unreleased]` — was rejected as a circuit breaker on the flaw rather than a repair of it.
