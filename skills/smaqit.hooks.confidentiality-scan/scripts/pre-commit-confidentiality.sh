#!/usr/bin/env bash
# smaqit.hooks.confidentiality-scan/scripts/pre-commit-confidentiality.sh
#
# Bundled copy of installer/hooks/pre-commit-confidentiality.sh, kept
# byte-for-byte in sync with it. It exists so this skill's manual-invocation
# path works standalone — in a clone that has never run `smaqit-extensions
# init`/`update`, there is no installed .smaqit/hooks/ copy to fall back on.
# If a project HAS installed the hook, .smaqit/hooks/pre-commit-confidentiality.sh
# is the authoritative one (force-overwritten on every install; this bundled
# copy is not touched by project installs, only by this skill's own updates).
# To exclude a path, add it to .smaqit/hooks/confidentiality-scan-ignore
# (present once the hook is installed) or run this script from a directory
