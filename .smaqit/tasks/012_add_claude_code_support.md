...
This mirrors a refactor already completed and proven in the sibling `smaqit` repo (`~/projects/smaqit`, commit `11be9be` "feat: add Claude Code support alongside GitHub Copilot (dual-target)", plus follow-up `e04300f`), which went through three rejected design iterations before landing on: **body/content stays in the existing source tree; only per-platform frontmatter is split out; all compiled/target-specific output lives in gitignored build directories, never committed.** `~/projects/iodis-crm-poc` contains an already-installed `.claude/` tree that shows the target shape in practice (and a few rot spots — see Pitfalls to Avoid below).
...
- **Claude naming convention — verified directly against `~/projects/smaqit/scripts/generate-agents.py` (not just the iodis-crm-poc reference, which turned out to diverge from the actual generator on this point):**
  ...
  - The `iodis-crm-poc` reference's hyphenated `.claude/skills/smaqit-session-start/` style directories do **not** match this — treat that repo as a divergent/manual port on this specific point, not the convention to follow.
...
## Pitfalls to Avoid (from iodis-crm-poc reference)

The `iodis-crm-poc` `.claude/` tree is a real but imperfect port and should not be copied uncritically:
...
- [x] Claude-side skill frontmatter preserves `metadata.version` (does not repeat the iodis-crm-poc regression) — confirmed: `generate_skills()` never touches skill frontmatter, only a text substitution for the placeholder
